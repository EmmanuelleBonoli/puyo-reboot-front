import {
    type Bubble, type BubblePosition,
    COLS_GRID_GAME,
    DIRECTIONS_GAME,
    type GridGame,
    MIN_MATCHING_BUBBLE, NEXT_ORIENTATION_MAP,
    ROWS_GRID_GAME, SATELLITE_OFFSETS
...
export function isInsideGrid(position: BubblePosition, grid: GridGame): boolean {
    return (
        position.rowIndex >= 0 &&
        position.rowIndex < grid.length &&
        position.columnIndex >= 0 &&
        position.columnIndex < grid[0].length
    );
}
...
export function computeGravityMovements(bubblesByCol: Record<number, Bubble[]>): Bubble[] {
    ...
    for (const col in bubblesByCol) {
        const columnBubbles = bubblesByCol[col].sort(
            (a, b) => a.position.rowIndex - b.position.rowIndex
        );
...
export function computeGravityDistances(
    updated: Bubble[],
    original: Bubble[]
    ...
    for (const bubble of updated) {
        ...
        if (originalBubble) {
            distanceMap[bubble.id] =
                bubble.position.rowIndex - originalBubble.position.rowIndex;
        }
    }
    return distanceMap;
}
